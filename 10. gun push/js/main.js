/* ============================================================
   ...
   8. Gün: Hata analizi yapıldı, performans ve validasyon düzeltmeleri eklendi
   ============================================================ */
...
/* ============================================================
   ...
   8. Gün HATA DÜZELTMESİ: "Gönder" butonuna bastığımda validasyon
   atlanıp form direkt gönderiliyordu. form submit event'ini dinleyerek
   düzelttim. onsubmit="return validateWithJS()" yetmiyordu çünkü
   return değeri false döndürmüyordu — boolean return ekledim.
   ============================================================ */
function validateWithJS() {
    ...
    if (errors.length > 0) {
        ...
        // Hata varsa false döndür — form gönderilmesin
        return false;
    } else {
        ...
        // Hata yoksa true döndür — form gönderilsin
        return true;
    }
}

/* "Gönder" butonuna basınca da validasyon çalışsın diye form submit'e bağladım.
   7. günde sadece buton onclick'i vardı, submit event'i yoktu.
   Bu yüzden Enter tuşuyla ya da direkt submit ile validasyon atlanıyordu. */
(function initContactFormSubmit() {
    const form = document.getElementById('contactForm');
    if (!form) return;
    form.addEventListener('submit', function (e) {
        // Validasyon başarısızsa formu durdur
        if (!validateWithJS()) {
            e.preventDefault();
        }
    });
...
/* ============================================================
   Login Hata Mesajı – URL Parametresinden Oku
   8. Gün HATA DÜZELTMESİ: login.html dosyasında <?php ?> kodu
   kullanmıştım ama .html uzantılı dosyada PHP çalışmaz!
   Hata mesajı hiç gösterilmiyordu. PHP yerine JS ile URL
   parametresini okuyarak düzelttim.
   ============================================================ */
(function initLoginError() {
    // Sayfanın adı login.html ise çalışsın
    if (!location.pathname.includes('login')) return;

    const params = new URLSearchParams(window.location.search);
    const hata = params.get('hata');
    const errorDiv = document.getElementById('loginHataDiv');

    if (!errorDiv) return;

    // PHP'den gelen yönlendirme parametrelerine göre mesaj göster
    if (hata === 'yanlis') {
        errorDiv.innerHTML = '<i class="bi bi-exclamation-triangle me-2"></i>Kullanıcı adı veya şifre hatalı. Lütfen tekrar deneyin.';
        errorDiv.style.display = 'block';
    } else if (hata === 'bos') {
        errorDiv.innerHTML = '<i class="bi bi-exclamation-triangle me-2"></i>E-posta ve şifre alanları boş bırakılamaz.';
        errorDiv.style.display = 'block';
    } else if (hata === 'csrf') {
        // 10. Gün: CSRF token hatası için mesaj eklendi
        errorDiv.innerHTML = '<i class="bi bi-shield-x me-2"></i>Güvenlik token\'ı geçersiz. Sayfayı yenileyip tekrar deneyin.';
        errorDiv.style.display = 'block';
    }
})();
...
/* ============================================================
   CSRF Token – 10. Gün
   login.html ve iletisim.html statik .html dosyaları olduğu için
   PHP kodu çalıştıramazlar. Token sunucudan AJAX ile alınıp
   formların hidden alanına yazılıyor.
   ============================================================ */
(function initCsrfTokens() {
    const loginInput   = document.getElementById('csrfTokenLogin');
    const contactInput = document.querySelector('#contactForm [name="csrf_token"]');

    if (!loginInput && !contactInput) return;

    fetch('php/csrf-token.php')
        .then(function(r) { return r.json(); })
        .then(function(data) {
            if (loginInput)   loginInput.value   = data.token;
            if (contactInput) contactInput.value = data.token;
        })
        .catch(function() {
            // CSRF endpoint ulaşılamazsa (örn. PHP olmayan ortam) form çalışmaya devam eder
            console.warn('CSRF token alınamadı — PHP sunucusu çalışıyor mu?');
        });
})();
...
/* ============================================================
   ...
   8. Gün PERFORMANS DÜZELTMESİ: Her mousemove event'inde doğrudan
   style değiştirmek çok fazla reflow tetikliyordu ve sayfayı
   yavaşlatıyordu. requestAnimationFrame ile throttle ettim.
   Arkadaşım bunu söylemişti, sonunda anlayabildim.
   ============================================================ */
(function initCursorGlow() {
    ...
    // Mouse pozisyonunu tut, RAF içinde uygula — her frame'de bir kez render
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        // Zaten bir RAF varsa ikinci kez schedule etme
        if (!rafScheduled) {
            rafScheduled = true;
            requestAnimationFrame(() => {
